...
- **Consumers:** `kortix tunnel ls|show|rpc|rm` CLI (PAT auth); the in-sandbox `agent-tunnel` opencode skill-CLI (resolves first-online tunnel via GET /connections, calls POST /rpc with the sandbox apiKey); the npm daemon `npx @vaelonx/agent-tunnel connect` (device-auth → WS); the web **Customize → Computers** surface (gated by the `agent_tunnel` experimental flag).
