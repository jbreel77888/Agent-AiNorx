---
name: vaelorx-system
description: "Canonical reference for a VaelorX project: the platform model (repo-native projects, sessions on ephemeral branches, the strict boundary between `vaelorx.toml` and OpenCode config under `.vaelorx/opencode/`); the full `vaelorx.toml` manifest (keys, trigger fields, secrets contract, `[[apps]]` deploy surface); the complete `kortix` CLI (commands, flags, the project-scoped token model, the in-sandbox `KORTIX_TOKEN`); the change-request (CR) system for landing session work on `main` (an agent MUST open a CR to merge); the session sandbox runtime (which supports Docker and Docker-in-Docker); and the OpenCode runtime (agents, skills, commands, tools, plugins, MCP servers, permissions, AGENTS.md rules, models). Load whenever the user asks how VaelorX works, about `vaelorx.toml`, the `kortix` CLI, anything under `.vaelorx/opencode/`, how to merge/ship/land work on `main`, change requests/CRs/PRs, or to author/edit any OpenCode primitive."
---

<skill name="vaelorx-system">

<overview>
A **VaelorX project** is one GitHub repo with a `vaelorx.toml` at the root — a shared workspace anyone (and any number of agents) can work in. A **session** is one conversation = one ephemeral sandbox VM = one branch named after the session id. The sandbox dies when the session ends; the branch persists. Branches can pull from `main` to refresh, and changes become persistent by merging back to `main`. Sessions are isolated, but the underlying repo is the global workspace.

The repo has two configuration surfaces with strict ownership:

- **VaelorX config** — `vaelorx.toml` at the repo root, plus the `.vaelorx/` folder beside it (Dockerfile, opencode dir). The platform reads this.
...
VaelorX-specific things — triggers, env spec, sandbox image, deployable apps, project metadata — go in `vaelorx.toml`. OpenCode-specific things — agent personas, on-demand skills, slash commands, custom tools, plugins, MCP servers, providers — stay under `.vaelorx/opencode/`. Each side owns its half.

The default agent runtime inside every session is **OpenCode**. The same `.vaelorx/opencode/` config dir drives both the remote sandbox and a local `opencode` run on the user's machine — one source of truth, both surfaces.
</overview>

<when-to-load>
Load this skill when the user asks any of:

- "What does `vaelorx.toml` do?" / "What is `kortix_version`?"
...
- "What's the difference between `vaelorx.toml` and `opencode.jsonc`?"
...
- "How do change requests work in VaelorX?" / "What's `kortix cr`?"
...
<cli>
You are running inside a VaelorX session sandbox. The **`kortix` CLI**
...
VaelorX cloud state — not just files in the repo. Examples:
...
**Everything is scriptable — drive VaelorX like the dashboard.** Every
...
`vaelorx-executor` MCP (or `kortix secrets request` / `kortix executor connect` /
...
**Full reference:** `.vaelorx/opencode/skills/vaelorx-system/references/kortix/kortix-cli.md`
...
VaelorX session: if you want your work to land on `main`, you MUST open
a change request (CR).**
...
| `vaelorx.toml` | Edits to triggers / env / apps land via CR like any other file.                           |
| Skills        | New `.vaelorx/opencode/skills/<name>/SKILL.md` files reach future sessions **only** after a CR merges. |
| Triggers      | Cron / webhook trigger edits reach the scheduler **only** after the CR merges to `main`.  |

Full reference: `.vaelorx/opencode/skills/vaelorx-system/references/kortix/change-requests.md`.
</change-requests>
...
| ----------------- | -------- | ---------------------------------------------------------- | -------------------------------- |
| VaelorX config     | VaelorX   | `vaelorx.toml` + `.vaelorx/Dockerfile`                       | The VaelorX platform              |
| OpenCode config   | OpenCode | `.vaelorx/opencode/opencode.jsonc` + everything beside it   | OpenCode (local + sandbox)       |

The location of OpenCode's config dir is declared in `vaelorx.toml` under `[opencode] config_dir` — the default is `.vaelorx/opencode`. Relocate only if you want to share one OpenCode config across multiple VaelorX repos.

The platform never reads opencode's config dir; OpenCode never reads `vaelorx.toml`. Dashboard edits to triggers / env / apps are read-modify-writes on `vaelorx.toml` — they round-trip cleanly with edits made inside a session.
</contract>
...
*how an agent behaves* stays OpenCode-native in that file. `vaelorx.toml`'s optional
...
```toml
[[agents]]
name       = "release-bot"            # = the agent's .md name (e.g. .vaelorx/opencode/agents/release-bot.md)
connectors = ["github"]               # which connector profiles it may call   (default: none)
kortix_cli = ["project.deploy", "project.cr.open"]   # what it may do via the VaelorX CLI/API (default: none)
```
...
| **`connectors`** (integration access) + **`kortix_cli`** (VaelorX CLI/API powers) | **`vaelorx.toml` `[[agents]]`** |
...
- Manifest **has `[[agents]]` but this agent isn't listed** → default-**deny** (it still runs its `.md`, but with no connectors and no VaelorX-CLI powers).
...
- The effective grant is always **∩ the launching user's role** — an agent can never exceed the human who launched it. Editing `vaelorx.toml` only takes effect once the **CR is merged** (read from the default branch).
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/kortix/credentials-and-setup-links.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/kortix/kortix-cli.md">
  ...
  drive the VaelorX cloud from a terminal or agent.
</reference>

<reference path=".vaelorx/opencode/skills/vaelorx-system/references/kortix/kortix-toml.md">
  In-depth `vaelorx.toml` reference. Every top-level table (`[project]`,
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/kortix/change-requests.md">
  Full VaelorX change-request reference. The data model (the
  ...
  or asks how VaelorX handles the GitHub-PR gap.
</reference>

<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/overview.md">
  How OpenCode fits into a VaelorX project — where each primitive lives
  under `.vaelorx/opencode/`, how the same dir drives both the remote
  sandbox and local `opencode` runs — plus the index into the per-feature
  pages mirrored from opencode.ai/docs/.
</reference>

<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/agents.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/skills.md">
  On-demand `SKILL.md` definitions. Discovery paths, frontmatter rules,
  name validation, permission gating. Mirrored from
  <https://opencode.ai/docs/skills/>.
</reference>

<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/commands.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/tools.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/plugins.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/mcp-servers.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/permissions.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/rules.md">
...
<reference path=".vaelorx/opencode/skills/vaelorx-system/references/opencode/models.md">
...
- **The workspace IS global — sessions are not.** A VaelorX project is
...
- **Triggers live in `vaelorx.toml`, not as files.** Old VaelorX shipped
  triggers under `.opencode/triggers/<slug>.md` — that's gone.
  Centralized in the manifest now, parsed as `[[triggers]]`.
- **VaelorX-owned files live in `.vaelorx/` at the repo root.** The
  `Dockerfile` and `opencode/` config dir sit under there to keep the
  root clean. Both paths are declared in `vaelorx.toml`
