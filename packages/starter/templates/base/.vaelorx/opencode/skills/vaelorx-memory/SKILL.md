...
`.vaelorx/memory` with the `memory` tool to read the index and recover
...
any time, so what isn't written to `.vaelorx/memory/` is lost.

Memory is **continuously CRUD'd**:

- Regular sessions add or update memory whenever they discover
  something durable worth keeping.
- The `memory-reflector` agent (`.vaelorx/opencode/agents/memory-reflector.md`)
...
same content. Edits land on `main` only via a VaelorX change request —
...
- Anything that's already in `vaelorx.toml`, `AGENTS.md`, or a SKILL.md.
- Secrets, tokens, API keys, PII — those live in the VaelorX Secrets
...
`.vaelorx/memory/` — not the generic `read` / `edit` / `write` tools.
...
Paths are repo-relative and must start with `.vaelorx/memory`
(e.g. `.vaelorx/memory/overview.md`). Writes go straight into the real
`.vaelorx/memory/` folder, so they show up in the working tree and land
on `main` through the normal change-request flow (below).

1. `view` `.vaelorx/memory` to see what's there.
...
only via a VaelorX change request — same path as any code change:
...
(`.vaelorx/opencode/agents/memory-reflector.md`) is a thin wrapper
...
4. CRUD `.vaelorx/memory/` accordingly.
...
named `memory-reflector` in `vaelorx.toml`. The cron sweep picks up
...
- **Don't put secrets in memory.** Memory is read into context and
  ...
  the VaelorX Secrets Manager, surfaced as env vars at runtime — not in
  `.vaelorx/memory/`.
