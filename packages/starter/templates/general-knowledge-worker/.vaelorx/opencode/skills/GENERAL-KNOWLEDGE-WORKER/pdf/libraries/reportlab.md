...
Always set metadata on every PDF you create. The author MUST be `"VaelorX"` and the title MUST describe the document contents.

- **Canvas**: call `c.setTitle(...)` and `c.setAuthor("VaelorX")` immediately after creating the canvas
- **SimpleDocTemplate**: pass `title=...` and `author="VaelorX"` as constructor kwargs
