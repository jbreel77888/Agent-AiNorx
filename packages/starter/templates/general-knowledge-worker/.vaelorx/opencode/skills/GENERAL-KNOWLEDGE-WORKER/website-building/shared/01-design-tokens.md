...
### VaelorX Design System (Fallback Palette — ONLY for VaelorX-branded sites)

This palette is ONLY for sites that are explicitly VaelorX-branded. For everything else, run the palette generator.

```css
/* NEXUS DESIGN SYSTEM — warm beige surfaces, teal primary accent */

:root, [data-theme="light"] {
  /* Surfaces (VaelorX Beige) */
  --color-bg:             #f7f6f2;
...
- `color-mix(in oklab, ...)` for opacity adjustments. Custom palettes: replace VaelorX defaults but keep variable names + both modes.
...
### VaelorX HSL Equivalents (for Tailwind / shadcn projects)

When using the fullstack webapp template (Tailwind + shadcn), `index.css` uses HSL values in `H S% L%` format (no `hsl()` wrapper). Below are VaelorX HSL conversions for when VaelorX is the appropriate fallback (see "Art Direction First" above). For inferred or custom palettes, convert your chosen colors to the same `H S% L%` format:
