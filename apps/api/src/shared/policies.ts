/**
 * Policy parsing for `kortix.toml` — the `[[policies]]` array and
 * `[policy]` settings block.
 *
 * This is the canonical home for policy parsing. Originally lived in
 * `projects/policies.ts`; moved here in Phase 7.0.4 so the executor can
 * consume it without depending on the (deprecated) projects/ module.
 *
 * Policies span EVERY connector in the project — patterns are
...
import { MANIFEST_FILENAME, type ParsedManifest } from './manifest';
